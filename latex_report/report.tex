...
\twocolumn[
\begin{center}
    {\LARGE\bfseries Gemini Powered Assessible Grasp Planning Pipeline: OOP}\vspace{4pt}\\
    {\large Abdul Azeem Makarim 24178227, Aftab Shiraz 23089652}\vspace{2pt}\\
    {\normalsize COMP0213 – Object Oriented Programming for Robotics and AI}\vspace{2pt}\\
    {\normalsize University College London, December 2025}\vspace{2pt}\\
    {\small \textbf{Repository:} \url{https://github.com/A-makarim/COMP0213_Group21}}\vspace{6pt}
\end{center}
]
...
\begin{lstlisting}
(instructions given verbally for acessibility)
...
\begin{subfigure}[b]{0.48\linewidth}
    \includegraphics[width=\linewidth]{figures/gripper_pr2_cuboid.png}
    \caption{PR2-Cuboid}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\linewidth}
    \includegraphics[width=\linewidth]{figures/gripper_pr2_cylinder.png}
    \caption{PR2-Cylinder}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\linewidth}
    \includegraphics[width=\linewidth]{figures/gripper_sdh_cuboid.png}
    \caption{SDH-Cuboid}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\linewidth}
    \includegraphics[width=\linewidth]{figures/gripper_sdh_cylinder.png}
    \caption{SDH-Cylinder}
\end{subfigure}
\caption{URDF for objects are generated from classes. PR2 is provided by the library. SDH was provided in week 1 installation folder}
\end{figure}
...
GRIPPER_CONFIG = {
    ...
    "sdh": {
        ...
        "roll_range": (-math.pi, math.pi), # full roll
...
\textbf{Radial noise:} $r = r_{\text{base}} + \mathcal{U}(r_{\text{min}}, r_{\text{max}})$ where uniform distribution used (not Gaussian) to ensure coverage of full approach distance range. PR2 samples $r \in [0.18, 0.38]$ m for cuboids. Note that this also accounts for noise in x axis and needs not to be repeated.
...
Below are the plots for training data distribution. One unique pattern in SDH roll axis dual patches for successes. With more training data, the successes and failures would form a boundary like a sin plot. this is because optimal roll intervals for SDH occur at every 90 degrees.
...
\hfill
\begin{subfigure}[b]{0.48\linewidth}
    \includegraphics[width=\linewidth]{figures/3d_visualization_pr2_cylinder.png}
    \caption{PR2-Cylinder: Wide success region}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\linewidth}
    \includegraphics[width=\linewidth]{figures/3d_visualization_sdh_cuboid.png}
    \caption{SDHCuboid: Tight success manifold}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\linewidth}
...
Oscillating patterns are clearly visible in roll distribution in both PR2 and SDH, almost tracing a sinusoidal  curve. Why are SDH plots skewed towards right side? The answer lies in the geometry of fingers of grippers. Because SDH has two fingers in parallel on one side, it can make it easier, or harder for it so grasp the object depending on other noise factors.

\begin{figure}[H]
\centering
\begin{subfigure}[b]{0.48\linewidth}
    \includegraphics[width=\linewidth]{figures/training_roll_distribution.png}
    \caption{roll distribution}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\linewidth}
    \includegraphics[width=\linewidth]{figures/training_pitch_distribution.png}
    \caption{pitch distribution}
\end{subfigure}
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Z-axis stratification:} Top grasps (z more than 0.4) mostly fail—fingers collide with object top before closing
...
\caption{Dataset distribution: more than 10,370 total samples across 4 configurations}
\end{figure}
...
\textbf{Automated batch generation:} To generate 9,770 training samples efficiently, we implemented automated batch processing for TMUX sessions:
...
\toprule
\textbf{Strategy} & \textbf{Accuracy} & \textbf{Precision} &
\textbf{Recall} & \textbf{F1} & \textbf{Bias} \\
\midrule
None (default) & 88.1\% & 0.92 & 0.61 & 0.73 & Majority \\
balanced & 87.3\% & 0.87 & 0.80 & 0.83 & Balanced \\
...
\textbf{Problem:} "least populated class has only 3 members when introdusing a class for partial success"
...
\textbf{Adding New Gripper (e.g. BarrettHand):}
\begin{lstlisting}
...
\textbf{Performance:} 240Hz simulation runs at 0.8× real-time on RTX 4060 (1.25s per 1s simulated). Acceptable for overnight batch generation.
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/emin.jpeg}
...
\textbf{Text-to-speech:} Offline synthesis via \texttt{pyttsx3}. Splits long responses (500 chars or more) into chunks to prevent TTS buffer overflow. Adjustable rate (150 wpm) and volume (90\%).
